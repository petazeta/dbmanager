...
export async function dbInitDb(){
  ...
  let sql=fs.readFileSync('./server/utils/' + dbConfig.dbsys + 'db.sql', 'utf8');
  if (dbConfig.dbsys=="pgsql") {
    sql += fs.readFileSync('./server/utils/' + dbConfig.dbsys + 'dbsync.sql', 'utf8');
  }
...
export async function dbUpdateQuery(sql){
  ...
  else if (dbConfig.dbsys=="pgsql") return (await dbQuery(sql)).rowCount;
}
